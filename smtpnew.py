...
root.iconbitmap("C:\\Users\\Lenovo\\Desktop\\codes\\cns\\IMAGES\\favicon (1).ico")
root.configure(bg="#323232")
#function which is called when submit button is pressed 
def send():
    try:      #try catch block if message is send then no error or else it will execute except block of statements 
        ...
        print(e)
#function is called when reset button is clicked and it deletes all the text present in the text box 
def reset():
...
#labels are like the text which u see on screen 
...
#entries for the mail
usernameentr = Entry(root , textvariable= username , font=('Calibri' , 15),bd=5)
usernameentr.place(x= 750 , y =200, height=35, width=400 ,anchor="center")  #entry_box.place(height=40, width=100)

passentr = Entry(root , textvariable=password ,show="*" , font=('Calibri' , 15),bd=5)
...
#buttons
sendbutton= Button(root,text="Send mail" , command=send , fg="#FA4EAB", font=('Calibri' , 20)).place(x= 750 , y =600, height=35, width=300 ,anchor="center")
resetbutton= Button(root,text="Reset mail" , command=reset , fg="#FA4EAB", font=('Calibri' , 20)).place(x= 750 , y =660, height=35, width=300 ,anchor="center")
root.mainloop()
